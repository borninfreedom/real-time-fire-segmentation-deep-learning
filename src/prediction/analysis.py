...
def save_image_prediction(image: np.ndarray, mask: np.ndarray,
                          resize_shape: Optional[Tuple[int, int]] = None,
                          save_path: str = "prediction.png"
                          ) -> None:
    """
    Plot an image, the predicted segmentation mask and the highlighted
    ROI of the segmentation mask, and save the result as a PNG file.
    ...
    save_path : str, optional
        Path to save the resulting image. Default is "prediction.png".
    """
    _, axes = plt.subplots(1, 3, figsize=(15, 8))
    ...
    legend_elements = [
        Patch(facecolor='r', edgecolor='black', label='Fire ROI')]
    ax.legend(handles=legend_elements, loc='upper left')
    title = 'An image along with the predicted fire mask and the highlighted segmentation'
    plt.suptitle(title)
    plt.tight_layout()
